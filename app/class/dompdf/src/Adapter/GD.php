...
class GD implements Canvas
{
    ...
    public function __construct($size = 'letter', $orientation = "portrait", Dompdf $dompdf, $aa_factor = 1.0, $bg_color = array(1, 1, 1, 0))
    {
        ...
        if (!is_array($size)) {
            ...
            if (isset(\app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES[$size])) {
                $size = \app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES[$size];
            } else {
                $size = \app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES["letter"];
            }
        }
