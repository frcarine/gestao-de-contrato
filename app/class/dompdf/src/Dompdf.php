 ...
 */
namespace app\class\dompdf\src;

use app\class\dompdf\lib\phpuse app\class\dompdf\src\Canvas;use app\class\dompdf\src\CanvasFactory;use app\class\dompdf\src\Exception;use app\class\dompdf\src\FontMetrics;use app\class\dompdf\src\Frame;use app\class\dompdf\src\FrameReflower\AbstractFrameReflower;use app\class\dompdf\src\Helpers;use app\class\dompdf\src\Options;use app\class\dompdf\src\Renderer;use DOMDocument;
use DOMNode;
use app\class\dompdf\src\Adapter\CPDF;
use DOMXPath;
use app\class\dompdf\src\Frame\Factory;
use app\class\dompdf\src\Frame\FrameTree;
use app\class\dompdf\lib\html5lib\HTML5_Tokenizer;
use app\class\dompdf\lib\html5lib\HTML5_TreeBuilder;
use app\class\dompdf\src\Image\Cache;
use app\class\dompdf\src\Renderer\ListBullet;
use app\class\dompdf\src\Css\Stylesheet;
...
class Dompdf
{
    ...
    */
    ...
    public function loadHtml($str, $encoding = 'UTF-8')
    {
        ...
        set_error_handler(array("\\app\\class\\dompdf\\src\\Helpers", "record_warnings"));
            ...
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    $decoded_string = preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/",
                        function ($matches) { return \app\class\dompdf\src\Helpers::unichr(hexdec($matches[1])); },
                        $style->content);
    ...
    public function getPaperSize($paperSize = null)
    {
        ...
        if (is_array($size)) {
            return $size;
        } else if (isset(\app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES[mb_strtolower($size)])) {
            return \app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES[mb_strtolower($size)];
        } else {
            return \app\class\dompdf\src\Adapter\CPDF::$PAPER_SIZES["letter"];
        }
    }
