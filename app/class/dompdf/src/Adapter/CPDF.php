...
namespace app\class\dompdf\src\Adapter;

use app\class\dompdf\src\Css\Style;
use app\class\dompdf\src\Canvas;
use app\class\dompdf\src\Dompdf;
use app\class\dompdf\src\Helpers;
use app\class\dompdf\src\Exception;
use app\class\dompdf\src\Image\Cache;
use app\class\dompdf\src\PhpEvaluator;
 ...
 * on the underlying {@link \app\class\dompdf\lib\Cpdf} class.
 *
 * @package dompdf
 */
class CPDF implements Canvas
{
    ...
    public function __construct($paper = "letter", $orientation = "portrait", Dompdf $dompdf)
    {
        ...
        $this->_pdf = new \app\class\dompdf\lib\Cpdf(
            $size,
            true,
     ...
     *
     * @return \app\class\dompdf\lib\Cpdf
     */
     ...
     * @param int $object
     * @param string $where
     *@see \app\class\dompdf\lib\Cpdf::addObject()
     *
     */
     ...
     *@see \app\class\dompdf\lib\Cpdf::setLineTransparency()
     *
     ...
     * @param string $mode the blending mode to use
     * @param float $opacity 0.0 fully transparent, 1.0 fully opaque
     *@see \app\class\dompdf\lib\Cpdf::setFillTransparency()
     *
     * Valid blend modes are (case-sensitive):
     *
     * Normal, Multiply, Screen, Overlay, Darken, Lighten,
     ...
     * @param string $join
     * @param array $dash
     *@see \app\class\dompdf\lib\Cpdf::setLineStyle()
     *
     */
    ...
    protected function _convert_gif_bmp_to_png($image_url, $type)
    {
        ...
        if (!function_exists($func_name)) {
            if (!method_exists("app\class\dompdf\src\Helpers", $func_name)) {
